...
> **说明**：本文档是 `UniApp 文件编辑` 技能的参考文档，用于指导 API 接口代码的编写规范。当创建或修改 API 文件时，应结合主技能使用。

---

## 执行前验证

**⚠️重要**：在执行任何 API 创建或修改操作之前，必须先验证：

1. 验证目标文件路径是否符合项目规范（参考主技能的「文件规范」）
2. 验证文件命名是否符合规范
3. 确认是否需要创建新文件或修改现有文件
4. 验证通过后，再继续执行本指南的后续操作
